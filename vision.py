...
class Vision:
    ...
    def findEnemies(self, screenshot):
        screen = cv.cvtColor(screenshot, cv.COLOR_RGB2GRAY)
        ret, enemies = cv.threshold(screen, 250, 255, cv.THRESH_BINARY)
        cv.imshow('treshold enemies', enemies)
    ...
    def draw_circles(self, haystack_img, circles):
        ...
        for (center_x, center_y) in circles:
            ...
            cv.circle(haystack_img, (int(center_x), int(center_y)), 20, line_color, lineType=line_type)
