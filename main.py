...
vision_limestone = Vision('l2_gold1.jpg')
...
while(True):
    ...
    #botik = bot.BotAction(wincap.offset_x, wincap.cropped_y, wincap.w, wincap.h) 
    #botik.turn_camera(500)

    # do object detection
    #rectangles = vision_limestone.find(processed_image, 0.46)
    enemies = vision_limestone.findEnemies(screenshot)

    # draw the detection results onto the original image
    #output_image = vision_limestone.draw_rectangles(screenshot, rectangles)
    enemies_image = vision_limestone.draw_circles(screenshot, enemies)
    ...
    cv.imshow('Enemies', enemies_image )
